function FoFIMT_v10
    ...
    % Run button (always visible & on top)
...
function onRun(~,~)
    ...
    if mode == 1
        % --- Single-model surface plot (unchanged) ---
        k = 50;
        ...
        % --- BI/IMV per your LaTeX definitions ---
        ...
        % Behavior Index: (1/n) * Σ_j Zall(:,:,j)
        BI  = (1/n) * sum(Zall,3);
        % IMV: sqrt((1/n) * Σ_j (Zall(:,:,j) - BI).^2)
        ...
        if mode == 2
            ...
            M   = IMV;
            ttl = 'Inter-Model Variability (IMV)';
        end
...
function inst = configureInstance(defStruct,defS,defT,defN,defLam,defW,idx,total,lockStruct)
    ...
    nRows       = 6;
    rowH        = 25;
    spacing     = 60;             % vertical distance between rows
    bottomGap   = 120;            % extra bottom space
    ...
    % OK button (centered at very bottom, y = 10)
    btnW = 100; btnH = 30;
